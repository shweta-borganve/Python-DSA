...
def test_user_registration_and_persistence():
    from main import users, save_users
...
def test_ticket_booking_seat_reduction():
    from main import movies, bookings, save_movies, save_bookings
    ...
    for movie in movies:
        if movie["title"].lower() == movie_title.lower() and tickets <= movie["available_seats"]:
